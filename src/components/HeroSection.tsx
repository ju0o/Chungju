      ...
      <div className="relative grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <h1 className="max-w-[10.5ch] whitespace-pre-line font-[family-name:var(--font-heading)] text-[2.85rem] leading-[1.08] tracking-[-0.045em]">
              {settings.heroTitle}
            </h1>
            <p className="hero-copy max-w-[31ch] text-[0.99rem] text-[var(--foreground-soft)]">{settings.heroDescription}</p>
            <div className="grid gap-2 text-xs tracking-[0.03em] text-[var(--foreground-soft)]">
              <p>{settings.eventDate}</p>
              <p>{settings.eventPlace}</p>
              <p>{settings.operationHours}</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2 text-[11px] leading-5 text-[var(--foreground-soft)]">
        ...
        <div className="grid gap-3">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            <div className="rounded-[1.4rem] border border-[var(--line)] bg-[rgba(255,252,246,0.76)] p-4">
              <p className="text-[11px] font-semibold tracking-[0.16em] text-[var(--foreground-soft)]">SCENE 01</p>
              <p className="mt-2 text-sm leading-7 text-[var(--foreground)]">메인배너는 자동 재생으로 현장 분위기를 먼저 전달해 텍스트 피로도를 낮춥니다.</p>
            </div>
            <div className="rounded-[1.4rem] border border-[var(--line)] bg-[rgba(255,252,246,0.76)] p-4">
              <p className="text-[11px] font-semibold tracking-[0.16em] text-[var(--foreground-soft)]">SCENE 02</p>
              <p className="mt-2 text-sm leading-7 text-[var(--foreground)]">작가와 책 정보는 이미지와 함께 카드형으로 보여주어 빠르게 훑어볼 수 있게 정리합니다.</p>
            </div>
            <div className="rounded-[1.4rem] border border-[var(--line)] bg-[rgba(255,252,246,0.76)] p-4">
              <p className="text-[11px] font-semibold tracking-[0.16em] text-[var(--foreground-soft)]">SCENE 03</p>
